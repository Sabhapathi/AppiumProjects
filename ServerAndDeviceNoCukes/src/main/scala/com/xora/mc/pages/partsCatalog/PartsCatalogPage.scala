...
class PartsCatalogPage(val driver: WebDriver, val baseUrl: String)
  ...
  def getPartNames(): List[String] = {
    val partNameElXpath = PARTS_CATALOG_BASE_XPATH + "//tr/td[2]"
    val partNamesEls = driver.findElements(By.xpath(partNameElXpath)).asScala
    val partNameList = partNamesEls.map(_.getText.trim).toList
    partNameList
  }
