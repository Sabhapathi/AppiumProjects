...
class InvoicesSanityTest extends SeleniumPostLoginTest with LoadableElement with ShouldMatchers {
  ...
  @BeforeClass
  def deleteAutoPartsCreatedDuringPreviousRun(){
    var partsCatalogPage = securedPage.loadPartsCatalogPage
    val partList = partsCatalogPage.getPartNames()
    for (i <- 0 until partList.size) {
      if (partList(i).contains("AutoPart")) {
        val contextMenuPopup = partsCatalogPage.contextMenuOfPart(partList(i))
        partsCatalogPage = contextMenuPopup.acceptDeletePart()
        sleepTimeInSecond(minWaitTime)
        webDriver.navigate().refresh()
        partsCatalogPage.getPartNames() should not contain partList(i)
      }
    }
  }
