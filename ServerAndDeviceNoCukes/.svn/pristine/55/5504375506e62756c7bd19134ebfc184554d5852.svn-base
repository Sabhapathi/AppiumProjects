...
//  private var dateFilter: DateFilterTimePeriodPickerWidget = null
 ...
 private def findSubmittedFormRow(formName: String) ={
   val rowXpath = SUBMITTED_FORMS_TABLE_XPATH + "//tr[.//*[text()='"+formName+"']]"
   new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(By.xpath(rowXpath)))
    }

  def contextMenuOfSubmittedForm(formName: String) = {
    val formRowEl = findSubmittedFormRow(formName)
    formRowEl.click()
