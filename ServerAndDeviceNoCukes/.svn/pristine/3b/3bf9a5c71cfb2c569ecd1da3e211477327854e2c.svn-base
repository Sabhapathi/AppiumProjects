...
//    new JobsMapPage(driver, mcBaseUrl)
  ...
  private def selectDeleteOptionForAAJ (deleteOption:String) {
    if(!deleteOption.equals("NA"))  {
      deleteOption match {
        case "Only this Job" =>  driver.findElement(By.id(DELETE_AAJ_ONLY_THIS_JOB_BUTTON_ID)).click()
        case "All future jobs in this series" => driver.findElement(By.id(DELETE_AAJ_ALL_FUTURE_JOB_BUTTON_ID)).click()
        case "This Job and all future Jobs" => driver.findElement(By.id(DELETE_AAJ_CURRENT_AND_ALL_FUTURE_JOB_BUTTON_ID)).click()
      }
    }
  }
